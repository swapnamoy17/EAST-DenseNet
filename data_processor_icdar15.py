...
def get_text_file(image_file):
    ...
    #print(txt_file)
    txt_file_name = txt_file.split("/")[-1]         #Uncomment these both if using ICDAR15 or 13
    #print(txt_file_name)
...
def count_samples(FLAGS):
    if FLAGS.validation_data_path == "":
        if sys.version_info >= (3, 0):
            count=0
            for data_path in FLAGS.training_data_path:
                count = count + len([f for f in next(os.walk(data_path))[2] if f[-4:] == ".jpg"])
            return count*FLAGS.train_val_ratio
        else:
            count=0
            for data_path in FLAGS.training_data_path:
                count = count + len([f for f in os.walk(FLAGS.training_data_path).next()[2] if f[-4:] == ".jpg"])
            return count*FLAGS.train_val_ratio
