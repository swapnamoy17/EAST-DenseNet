...
def load_annotation(p):
    ...
    with open(p, 'r') as f:
        #print("Data processor p------------",p)
        reader = csv.reader(f)
        for line in reader:
            #print("Data processor line------------",line)
            label = line[-1]
            # strip BOM. \ufeff for python3,  \xef\xbb\bf for python2
            line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in line]
            x1, y1, x2, y2, x3, y3, x4, y4 = list(map(float, line[:8]))
            #print("Data processor line------------",x1, y1, x2, y2, x3, y3, x4, y4 )
...
def generator(FLAGS, input_size=512, background_ratio=3./8, is_train=True, idx=None, random_scale=np.array([0.5, 1, 2.0, 3.0]), vis=False):
    ...
    individual_list=[]
    for data_path in FLAGS.training_data_path:
        individual_list.extend(get_images(data_path))
        
    if FLAGS.validation_data_path=="":
        #validation data is in training folder
        #So split according to train: val ratio
        print("Splitting training dataset into training + validation in a ratio of :",FLAGS.train_val_ratio)
        splitIndex = int(FLAGS.train_val_ratio*len(individual_list))
        individual_list = individual_list[:splitIndex]
        
    image_list = np.array(individual_list)
...
def count_samples(FLAGS):
    if FLAGS.validation_data_path == "":
        if sys.version_info >= (3, 0):
            count=0
            for data_path in FLAGS.training_data_path:
                count = count + len([f for f in next(os.walk(data_path))[2] if f[-4:] == ".jpg"])+len([f for f in next(os.walk(data_path))[2] if f[-4:] == ".png"])
            return count*FLAGS.train_val_ratio
        else:
            count=0
            for data_path in FLAGS.training_data_path:
                count = count + len([f for f in os.walk(FLAGS.training_data_path).next()[2] if f[-4:] == ".jpg"])+len([f for f in os.walk(data_path).next()[2] if f[-4:] == ".png"])
            return count*FLAGS.train_val_ratio
...
def load_data(FLAGS, is_train=False):
    ...
    if FLAGS.validation_data_path=="":
        ...
        splitIndex = int(FLAGS.train_val_ratio*len(individual_list))
        individual_list = individual_list[splitIndex:]
        image_files = np.array(individual_list)
...
if __name__ == '__main__':
    text_polys = load_annotation('/content/drive/My Drive/EAST/ICDAR13/train2/gt_101.txt')
    print(text_polys)
    pass
