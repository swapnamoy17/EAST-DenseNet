...
def resize_bilinear1(x):
    return tf.image.resize_bilinear(x, size=[(K.shape(x)[1]+1)*RESIZE_FACTOR, (K.shape(x)[2]+1)*RESIZE_FACTOR])

def resize_bilinear2(x):
    return tf.image.resize_bilinear(x, size=[K.shape(x)[1]*RESIZE_FACTOR+1, K.shape(x)[2]*RESIZE_FACTOR+1])
...
class EAST_model:
    ...
    def __init__(self, input_size=512):
        ...
        inception = InceptionV3(input_tensor=input_image, weights='imagenet', include_top=False, pooling=None)
        x = inception.get_layer('activation_94').output

        x = Lambda(resize_bilinear1, name='resize_1')(x)
        x = concatenate([x, inception.get_layer('activation_70').output], axis=3)
        ...
        x = concatenate([x, inception.get_layer('activation_50').output], axis=3)
        ...
        x = Lambda(resize_bilinear2, name='resize_3')(x)
        x = concatenate([x, inception.get_layer('activation_4').output], axis=3)
