...
def generator(FLAGS, input_size=512, background_ratio=3./8, is_train=True, idx=None, random_scale=np.array([0.5, 1, 2.0, 3.0]), vis=False):
    ...
    if FLAGS.validation_data_path=="":
        ...
        splitIndex = int(FLAGS.train_val_ratio*image_list[0])
        image_list = image_list[:splitIndex+1]
    if not idx is None:
...
def load_data(FLAGS, is_train=False):
    if FLAGS.validation_data_path=="":
        ...
        splitIndex = int(FLAGS.train_val_ratio*image_files[0])
        image_files = image_files[splitIndex+1:]
    else:
