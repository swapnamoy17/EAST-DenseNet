...
def main(argv=None):
    ...
    for img_file in img_list:
        ...
        if FLAGS.dataset == "icdar13" or FLAGS.dataset == "icdar15":
            res_file = os.path.join(FLAGS.output_dir,'res_{}.txt'.format(os.path.basename(img_file).split('.')[0]))
        elif FLAGS.dataset == "mlt":
            res_file = os.path.join(FLAGS.output_dir,'res_{}.txt'.format(os.path.basename(img_file).split('.')[0]))
            res_file = res_file.replace("tr_","")
        else:
            print('Wrong dataset')
        # save to file
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if FLAGS.dataset == "icdar13":
                        f.write('{},{},{},{}\r\n'.format(
                            min(box[0, 0],box[1, 0],box[2, 0],box[3, 0]),min(box[0, 1],box[1, 1],box[2, 1],box[3, 1]),max(box[0, 0],box[1, 0],box[2, 0],box[3, 0]),max(box[0, 1],box[1, 1],box[2, 1],box[3, 1])
                        ))
                    elif FLAGS.dataset == "mlt" or FLAGS.dataset == "icdar15":
                        f.write('{},{},{},{},{},{},{},{}\r\n'.format(
    ...
    print("TOTAL TIME TAKEN FOR CREATING FILE FOR ONLY TASK1 EVALUATION: {}".format(proge))
