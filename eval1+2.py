...
parser.add_argument('--crop_dir', type=str, default='/content/drive/My Drive/EAST-master/cropped/')
...
def detect(score_map, geo_map, timer, score_map_thresh=0.8, box_thresh=0.1, nms_thres=0.2):
    ...
    if len(score_map.shape) == 4:
        score_map = score_map[0, :, :, 0]   #1 image per detct() func call then first axuis 0  and only 1 score so 4th axis 0
        geo_map = geo_map[0, :, :, ]       #1 image per detect() func call
...
def main(argv=None):
    ...
    if FLAGS.image_path=='':
        ...
        img_list=[]
        img_list.append(FLAGS.image_path)
        ...
        if boxes is not None:
            ...
            with open(res_file, 'w') as f:
                for box in boxes:
                    ...
                    cv2.drawContours(img[:, :, ::-1], [box], 0, (0, 0, 255), 2)
                    width = int(rect[1][0])
                    height = int(rect[1][1])
                    ...
                    warped = cv2.warpPerspective(img[:, :, ::-1], M, (width, height))
                    #print(warped.shape)
                    if warped.shape[1]<warped.shape[0]:
                      warped=cv2.rotate(warped,cv2.ROTATE_90_COUNTERCLOCKWISE)
                    crop_path=os.path.join(FLAGS.crop_dir, os.path.basename(img_file)).replace('.jpg','-{}.jpg'.format(i))
                    cv2.imwrite(crop_path,warped)
                    f.write('{},{},{},{},{},{},{},{},{}\r\n'.format(
                        box[0, 0], box[0, 1], box[1, 0], box[1, 1], box[2, 0], box[2, 1], box[3, 0], box[3, 1],crop_path
                    ))
    ...
    proge=time.time()-progs
    print('TOTAL TIME FOR CREATING FILES(COORDINATES WITH PATHS) FOR 500 TEST IMAGES:{}'.format(proge))
