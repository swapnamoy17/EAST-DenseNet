...
class EAST_model:
    ...
    def __init__(self, input_size=512):
        ...
        resnet = DenseNet201(input_tensor=input_image, weights='imagenet', include_top=False, pooling=None)
        x = resnet.get_layer('relu').output

        x = Lambda(resize_bilinear, name='resize_1')(x)
        x = concatenate([x, resnet.get_layer('pool4_relu').output], axis=3)
        ...
        x = concatenate([x, resnet.get_layer('pool3_relu').output], axis=3)
        ...
        x = concatenate([x, resnet.get_layer('pool2_relu').output], axis=3)
